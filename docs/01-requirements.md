...
| 用語 | 定義 |
|---|---|
| 顔パーツ | AI によって顔写真から切り出された部位画像。本システムでは向かって左眉・向かって右眉・向かって左目・向かって右目・鼻・口の計 6 パーツを対象とする |
...
- 説明:本システムは、撮影した顔写真に対して AI 顔検出処理を実行し、眉×2・目×2・鼻×1・口×1 の計 6 パーツを個別の画像パーツとして切り出さなければならない。
- 優先度:必須
- 関連 UC:UC-002
...
| ID | 判定基準 |
|---|---|
| AC-003 | 顔が 1 名含まれる写真に対して、主要パーツ(眉×2、目×2、鼻×1、口×1)の計 6 パーツが切り出されること |
...
- 説明:「目隠し風モード」選択時、本システムは以下のゲームフローを提供しなければならない。
  ...
  3. 画面中央に「[パーツ名]はどこですか？」のテキスト指示を表示する。対象パーツの順序は向かって左の眉・向かって右の眉・向かって左の目・向かって右の目・鼻・口の順とする。パーツ名はユーザ(操作者)から見た方向を基準として表現し、被写体視点の「右目」「左目」「右眉」「左眉」という表現は使用しない。
...
| AC-013 | 各パーツに対してテキスト指示が順次表示されること(向かって左の眉→向かって右の眉→向かって左の目→向かって右の目→鼻→口の順) |
...
| AC-019 | 生成されたのっぺらぼう画像において、顔パーツ(眉×2、目×2、鼻×1、口×1)の領域が除去されていること |
...
|---|---|---|
| 向かって左の眉 | 画面左側に表示される眉 | 被写体の右眉 |
| 向かって右の眉 | 画面右側に表示される眉 | 被写体の左眉 |
...
| ID | 判定基準 |
|---|---|
| AC-027 | システムの画面表示・指示テキストにおいて、パーツ名が「向かって左の眉」「向かって右の眉」「向かって左の目」「向かって右の目」の表現を使用していること |
| AC-028 | 「右目」「左目」「右眉」「左眉」という被写体視点の表現がシステムの UI 上に出現しないこと |
...
| 0.4.0 | 2026-05-10 | 眉毛パーツ追加(計 6 パーツ化)に伴う FR-002・FR-007・FR-009 の更新(AC-003・AC-013・AC-019・AC-027・AC-028 改定、FR-009 命名基準表に眉行追加) | requirements-coach |
